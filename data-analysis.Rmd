...
We can now simply plug in our data into the built-in student t test function in R. The default confidence interval, which we will choose for out test, is at 95%.

```{r}
mergeByDirection <- function(clockwise, counter_clockwise) {
  cw <- data.central %>%
    filter(road %in% clockwise, sample_size >= 30) %>%
    select(-c(sample_sd, sample_median, sample_mode))
  ccw <- data.central %>%
    filter(road %in% counter_clockwise, sample_size >= 30) %>%
    select(-c(sample_sd, sample_median, sample_mode)) %>%
    rename(
      sample_mean_ccw = sample_mean,
      sample_variance_ccw = sample_variance,
      sample_size_ccw = sample_size)
  ccw$road_ccw <- ccw$road
  ccw$road <- sapply(ccw$road, function(x){clockwise[ which(x == counter_clockwise) ]})
  return(merge(cw, ccw))
}

ring_road.mean_diff <- mergeByDirection(ring_road, ring_road_ccw) %>%
  mutate(
    difference_mean = sample_mean - sample_mean_ccw,
    difference_sd = sqrt(sample_variance/sample_size + sample_variance_ccw/sample_size_ccw)) %>%
  select(-c(sample_mean, sample_mean_ccw, sa))
```


## Hypothesis: Hourly congestion varies by weekday/weekend

We simply follow the same steps as before, except we cast the data in a different light.
...
## Correlation: Speed vs Congestion
