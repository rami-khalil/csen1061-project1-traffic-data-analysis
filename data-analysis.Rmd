...
Using the previously calculated central tendencies data:
...
mergeByDirection <- function(clockwise, counter_clockwise) {
  ...
  ccw <- data.central %>%
    ...
    select(-c(sample_sd, sample_median, sample_mode)) %>%
    rename(sample_mean_ccw = sample_mean, sample_variance_ccw = sample_variance, sample_size_ccw = sample_size)
  ccw$road_ccw <- ccw$road
  ccw$road <- sapply(ccw$road, function(x){clockwise[ which(x == counter_clockwise) ]})
  return(merge(cw, ccw))
}

ring_road.means <- mergeByDirection(ring_road, ring_road_ccw) %>%
  mutate(
