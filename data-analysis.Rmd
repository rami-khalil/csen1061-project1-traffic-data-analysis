...
data.speed %>%
  ...
  qplot(report_hour, speed, data = x, geom = c("boxplot"), main = "Ta7rir To Mohandesin")})

data.speed %>%
  filter(road == 164, weekend == FALSE) %>% (function(x) {
  qplot(report_hour, speed, data = x, geom = c("boxplot"), main = "Mohandesin To Ta7rir")})
```
...
It is convenient and reasonable to assume that weekday and weekend speed distributions are different, but let's try to confirm that assumption using our data.
